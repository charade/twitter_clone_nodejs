...
router.get('/',controller.logout, async (req,res)=>{
    const err = await req.consumeFlash("warning")
   res.render('login.ejs', {err});
})

///page signup
router.get('/signup',async(req,res)=>{
    const err_msg = await req.consumeFlash('userExist')
    res.render('signup.ejs',{err_msg});
})
...
router.get('/username',admin, controller.allUserTweets);
